...
void RoutingRewrite::operator()() const
{
	std::map<
	    ...
	    std::map<common::ChipOnConnection, std::vector<grenade::common::VertexOnTopology>>>
	...
	for (auto const& vertex_descriptor : get_topology().get_reference().vertices()) {
		...
		if (partitioned_vertex.get_time_domain()) {
			partitioned_vertices_per_chip_per_execution_instance
			    [execution_instance_on_executor][grenade::vx::common::ChipOnConnection()]
	...
	for (auto const& [execution_instance, partitioned_vertices_per_chip] :
	     partitioned_vertices_per_chip_per_execution_instance) {
		for (auto const& [chip, partitioned_vertices] : partitioned_vertices_per_chip) {
			auto const& local_routing_result = routing_result.chips.at(execution_instance)
			                                       .at(grenade::vx::common::ChipOnConnection());
				...
				if (auto const partitioned_projection =
					...
					if (auto const partitioned_uncalibrated_synapse =
						...
						for (auto const& [synram, placed_conns] : placed_connections) {
							...
							std::set<
							    signal_flow::vertex::SynapseArrayViewSparse::Columns::value_type>
							    columns;
							...
							for (auto const& placed_conn : placed_conns) {
								synapses.push_back(
								    signal_flow::vertex::SynapseArrayViewSparse::Synapse{
								        static_cast<size_t>(std::distance(
								            rows.begin(),
								            rows.find(
								                placed_conn.synapse_row.toSynapseRowOnSynram()))),
								        static_cast<size_t>(std::distance(
								            columns.begin(),
								            columns.find(placed_conn.synapse_on_row)))});
							...
							projections[execution_instance][partitioned_vertex_descriptor][synram] =
							    get_topology().add_vertex(
							        signal_flow::vertex::SynapseArrayViewSparse(
							            synram,
							...
							for (auto const out_edge_descriptor :
								...
								for (size_t cs = 0; auto const& column : columns) {
									if (auto const it = std::find(
										...
										channels_on_source.push_back(
										    grenade::common::MultiIndex({cs}));
			...
			for (auto const& partitioned_vertex_descriptor : partitioned_vertices) {
				...
				if (auto const partitioned_spike_recorder =
						...
						if (auto const* external_source_neuron =
							...
							for (size_t i = 0; i < source_channels.size(); i++) {
								local_labels.at(std::distance(
								    ...
								    std::find(
								        recorder_shape_elements.begin(),
								        recorder_shape_elements.end(), target_channels.at(i)))) =
					...
					for (auto const in_edge_descriptor :
						...
						size_t const cell_on_population_dimension =
						...
						// This maps neurons on the source population to their index on the
						// population, wich might differ (due to previous splitting of a larger
						// population).
						...
						for (auto const& in_edge_channel_on_source :
						     in_edge_channels_on_source.get_elements()) {
							auto const neuron_on_population =
							    in_edge_channel_on_source.value.at(cell_on_population_dimension);
							halco::hicann_dls::vx::v3::CompartmentOnLogicalNeuron
							    compartment_on_neuron(in_edge_channel_on_source.value.at(
							        compartment_on_neuron_dimension));
							auto const atomic_neuron_on_compartment =
							    in_edge_channel_on_source.value.at(
					...
					for (auto const& [synram, columns] : columns_per_synram) {
						mapped_vertex_descriptors.push_back(
						    get_topology().add_vertex(signal_flow::vertex::CADCMembraneReadoutView(
						        ...
						        common::ChipOnConnection(), time_domain.value(),
						        execution_instance)));
					}
					get_topology().add_inter_graph_hyper_edge(
					...
					for (auto const in_edge_descriptor :
						...
						auto const& in_edge_channels_on_source = in_edge.get_channels_on_source();
						for (auto const& in_edge_channel :
						     in_edge_channels_on_source.get_elements()) {
							auto const& neuron_on_population =
	...
	for (auto const& [execution_instance, partitioned_vertices_per_chip] :
		...
		for (auto const& [chip, partitioned_vertices] : partitioned_vertices_per_chip) {
			...
			for (auto const& partitioned_vertex_descriptor : partitioned_vertices) {
				...
				if (auto const population =
					...
					if (auto const external_source_neuron =
					    ...
					    external_source_neuron) {
						if (get_topology().get_reference().in_degree(
						        partitioned_vertex_descriptor) == 0) {
							continue;
						}
						for (auto const& in_edge_descriptor :
						     get_topology().get_reference().in_edges(
						         partitioned_vertex_descriptor)) {
							auto const source_descriptor =
							    get_topology().get_reference().source(in_edge_descriptor);
							auto const& source =
							    get_topology().get_reference().get(source_descriptor);
							...
							if (auto const source_population =
							    ...
							    source_population) {
								if (auto const delay_cell = dynamic_cast<DelayCell const*>(
								        &source_population->get_cell());
								    delay_cell) {
									for (auto const& delay_in_edge_descriptor :
										...
										if (auto const spike_recorder =
											...
											for (size_t c = 0;
											     c < channels_on_spike_recorder.size(); ++c) {
												auto const& channel_on_spike_recorder =
												    channels_on_spike_recorder.at(c);
												auto const& channel_on_delay =
												    channels_on_delay.at(c);
												...
												size_t const external_population_index =
												    std::distance(
												        output_channels_on_delay.begin(),
												        output_channel_on_delay_it);
												auto const& channel_on_population =
												    channels_on_external.at(
												        external_population_index);
											...
											get_topology().add_inter_graph_hyper_edge(
											    ...
											    {source_descriptor},
											    DelayCellMapping(
											        std::move(delay_cell_mapping_labels)));
											...
											throw std::runtime_error(
											    "Delay cell population getting input from vertex "
											    "other "
									...
									throw std::runtime_error(
									    "External source neuron population getting input from "
									    "other "
								...
								get_topology().add_inter_graph_hyper_edge(
								    ...
								    grenade::common::FixtureInterTopologyHyperEdge(
								        1, network::vertex::transformation::SpikeLookup::
								               Parameterization(std::move(translation))));
								...
								throw std::runtime_error(
								    ...
								    "population which is not a delay cell population  or vertex "
								    "which "
			...
			std::map<
			    halco::hicann_dls::vx::v3::SynapseDriverOnDLS, grenade::common::VertexOnTopology>
			...
			for (auto const& [synapse_driver_on_dls, compare_mask] :
				...
				for (auto const& [partitioned_vertex_descriptor, synapses] :
					...
					for (auto const& [synram, synapse_vertex_descriptor] : synapses) {
						...
						for (auto const& synapse_row : synapse_driver_on_dls.toSynapseRowOnDLS()) {
							if (auto const it = std::find(
							        synapse_vertex.get_rows().begin(),
							        synapse_vertex.get_rows().end(),
					...
					if (padi_bus_on_dls ==
					    halco::hicann_dls::vx::v3::PADIBusOnDLS(
					        synapse_driver_on_dls.toSynapseDriverOnSynapseDriverBlock()
					            .toPADIBusOnPADIBusBlock(),
					        synapse_driver_on_dls.toSynapseDriverBlockOnDLS()
					            .toPADIBusBlockOnDLS())) {
						get_topology().add_edge(
			...
			for (auto const& [crossbar_node_on_dls, crossbar_node_config] :
				...
				get_topology().add_inter_graph_hyper_edge(
				    ...
				    grenade::common::FixtureInterTopologyHyperEdge(
				        1,
				        signal_flow::vertex::CrossbarNode::Parameterization(crossbar_node_config)));

				if (crossbar_node_on_dls.toCrossbarInputOnDLS().toSPL1Address()) {
					if (crossbar_l2_inputs.contains(execution_instance)) {
						get_topology().add_edge(
						    crossbar_l2_inputs.at(execution_instance),
						    crossbar_node_vertex_descriptor,
						    grenade::common::Edge(
						        grenade::common::CuboidMultiIndexSequence({1}),
						        grenade::common::CuboidMultiIndexSequence({1})));
						...
						for (auto const& [_, neurons] :
							...
							for (auto const& [neuron_row_on_dls, neuron_vertex_descriptor] :
								...
								std::vector<grenade::common::MultiIndex>
								    columns_on_neuron_event_output;
								for (size_t c = 0;
								     auto const& column : neuron_vertex.get_columns()) {
				...
				if (background_spike_sources.contains(execution_instance)) {
					for (
					    auto const& [background_spike_source_on_dls, background_vertex_descriptor] :
			...
			for (auto const& partitioned_vertex_descriptor : partitioned_vertices) {
				if (auto const partitioned_plasticity =
					...
					std::map<
					    size_t, std::vector<signal_flow::vertex::PlasticityRule::SynapseViewShape>>
					    synapse_view_shapes_map;
					std::map<
					    size_t, std::vector<signal_flow::vertex::PlasticityRule::NeuronViewShape>>
					...
					for (auto const& in_edge :
								...
								neuron_view_shapes_map
								    [get_topology().get_reference().get(in_edge).port_on_target -
								     partitioned_plasticity->get_projection_shapes().size()]
								        .push_back(
								            signal_flow::vertex::PlasticityRule::NeuronViewShape{
						...
						if (auto const source_projection_section =
							...
							for (auto const& [synram_on_dls, synapse_vertex_descriptor] :
								...
								auto const& synapse_vertex = dynamic_cast<
								    signal_flow::vertex::SynapseArrayViewSparse const&>(
								    get_topology().get(synapse_vertex_descriptor));
								synapse_view_shapes_map
								    [get_topology().get_reference().get(in_edge).port_on_target]
								        .push_back(
					...
					std::vector<signal_flow::vertex::PlasticityRule::NeuronViewShape>
					    neuron_view_shapes;
					std::map<grenade::common::VertexOnTopology, size_t>
					    synapse_view_vertices_reverse;
					std::map<grenade::common::VertexOnTopology, size_t>
					    neuron_view_vertices_reverse;
					size_t i_synapse = 0;
					for (auto const& [synapse_view_index, local_synapse_view_shapes] :
						...
						for (size_t i = 0;
							...
							synapse_view_vertices_reverse[synapse_view_vertices[synapse_view_index]
							                                  .at(i)] = i_synapse;
							i_synapse++;
							i++;
						}
								...
								get_topology().add_edge(
								    ...
								    grenade::common::Edge(
								        ...
								        grenade::common::CuboidMultiIndexSequence(
								            {neuron_vertex.get_columns().size()}),
								        0,
								        num_synapses + neuron_view_vertices_reverse.at(
								                           neuron_vertex_descriptor)));
							}
						}
						if (auto const source_projection_section =
						        dynamic_cast<grenade::common::Projection const*>(&source_vertex);
								...
								get_topology().add_edge(
								    ...
								    grenade::common::Edge(
								        ...
								        grenade::common::CuboidMultiIndexSequence(
								            {synapse_vertex.get_columns().size()}),
								        0,
								        synapse_view_vertices_reverse.at(
								            synapse_vertex_descriptor)));
							}
						}
					}
