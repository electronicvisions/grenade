			...
			auto const filter = [&](auto const& connections) {
				...
				for (auto const& c : connections) {
					...
					auto const ipost = network.projections.at(c.descriptor.first)
					                    .connections.at(c.descriptor.second)
					                    .index_post;
					auto const& neuron_post = std::get<Population>(network.populations.at(ppost)).neurons.at(ipost);
					if (local_descriptors.contains(std::pair{pd, cd}) && (neuron_post.toNeuronRowOnDLS().toPADIBusBlockOnDLS() == padi_bus.toPADIBusBlockOnDLS())) {
						ret.push_back(c);
					}
				}
				return ret;
