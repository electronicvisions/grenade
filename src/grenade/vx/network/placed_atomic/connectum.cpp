#include "grenade/vx/network/placed_atomic/connectum.h"
	...
	for (auto const& [descriptor, projection] : network.projections) {
		...
		for (auto const& connection : projection.connections) {
			connectum.push_back(ConnectumConnection{
			    std::pair{projection.population_pre, connection.index_pre},
			    neurons_post.at(connection.index_post), projection.receptor_type,
			    connection.weight});
		}
	}
	return connectum;
