#include "grenade/vx/network/placed_atomic/routing/routing_constraints.h"

#include "grenade/vx/network/placed_atomic/network.h"
#include "hate/math.h"
#include <iostream>

namespace grenade::vx::network::placed_atomic::routing {
    ...
    const
{
	...
	for (auto const& [descriptor, projection] : m_network.projections) {
		...
		for (auto const& connection : projection.connections) {
			ret.push_back(
			    {population_pre.neurons.at(connection.index_pre),
			     population_post.neurons.at(connection.index_post),
			     {descriptor, i},
		...
		for (auto const& connection : projection.connections) {
			ret.push_back(
			    {BackgroundSpikeSourceOnDLS(
			         population_pre.coordinate.at(population_post.neurons.at(connection.index_post)
			                                          .toNeuronRowOnDLS()
			                                          .toHemisphereOnDLS()) +
			         population_post.neurons.at(connection.index_post)
			                 .toNeuronRowOnDLS()
		...
		for (auto const& [receptor_type, descriptors] :
			...
			for (auto const& descriptor : descriptors) {
				...
				ret[hemisphere][receptor_type].insert(
				    {projection.population_pre, connection.index_pre});
			}
		}
	}
	return ret;
	...
	for (auto const& [_, pop] : m_network.populations) {
		...
		for (size_t i = 0; i < population.neurons.size(); ++i) {
			if (population.enable_record_spikes.at(i)) {
				neurons.insert(population.neurons.at(i));
			}
		}
	}
...
RoutingConstraints::get_neither_recorded_nor_source_neurons() const
{
	...
	for (auto const& [_, pop] : m_network.populations) {
		...
		for (size_t i = 0; i < population.neurons.size(); ++i) {
			if (!population.enable_record_spikes.at(i)) {
				ret.insert(population.neurons.at(i));
			}
		}
	}
