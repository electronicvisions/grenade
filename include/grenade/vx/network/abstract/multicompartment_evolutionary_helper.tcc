...
void MutationShiftRound<EOT>::operator()(EoPopulation<EOT>& pop)
{
	...
	for (EOT& chrom : pop) {
		...
		x_shift = m_shift_distance(m_generator) % pop.size();
	...
	for (auto& chrom : pop) {
		...
		if (!m_together) {
			for (size_t i = 0; i < m_number_columns; i++) {
				...
				// Get insertion point
				x_modify = m_position(m_generator);

				// Shift genome from insertion point to right
				std::shift_right(begin(chrom) + 9 * x_modify, end(chrom), 9 * 1);

				// Fill new space with zeros
				for (size_t j = 9 * x_modify; j < 9 * (x_modify + 1); j++) {
					chrom[j] = 0;
				}
			}
		}
		// Place block of size #m_number_columns in genome of individuals
			...
			// Shift genome from insertion point to right
			std::shift_right(begin(chrom) + 9 * x_modify, end(chrom), 9 * m_number_columns);

			// Fill new space with zeros
			for (size_t j = 9 * x_modify; j < 9 * (x_modify + m_number_columns); j++) {
				chrom[j] = 0;
			}
		}
	}
	...
	for (auto& chrom : pop) {
		...
		if (!m_together) {
			for (size_t i = 0; i < m_number_columns; i++) {
				...
				// Shift genome from insertion point to left
		...
		// Place block of size #m_number_columns in genome of individuals
			...
			// Shift genome from insertion point to left
			std::shift_left(begin(chrom) + 9 * x_modify, end(chrom), 9 * m_number_columns);

			// Fill new space at end of chrom with zeros
			for (size_t j = chrom.size() - 9 * m_number_columns; j < chrom.size(); j++) {
				chrom[j] = 0;
			}
		}
	}
