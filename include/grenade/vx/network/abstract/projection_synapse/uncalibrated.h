 ...
 */
struct SYMBOL_VISIBLE GENPYBIND(visible) UncalibratedSynapse
    : public grenade::common::Projection::Synapse
{
	/**
	 * Weight value in units of the hardware synapse circuit property.
	 * Values larger than configurable using a single synapse are spread using multiple synapse
	 * circuits.
	 */
	struct GENPYBIND(inline_base("*")) Weight
	    : public halco::common::detail::BaseType<Weight, uintmax_t>
	{
		constexpr explicit Weight(value_type const value = 0) : base_t(value) {}
	};

	struct SYMBOL_VISIBLE ParameterSpace
	    : public grenade::common::Projection::Synapse::ParameterSpace
	{
		struct SYMBOL_VISIBLE Parameterization
	...
	/**
	 * UncalibratedSynapse requires a time domain, since it is represented by a physical circuit
	 * operating in time.
	 */
	...
	/**
	 * Get input ports.
	 * The UncalibratedSynapse features one input port of spikes to the synapses (port index 0) and
	 * one for the weight parameterization (port index 1).
	 */
	virtual Ports get_input_ports() const override;

	/**
	 * Get output ports.
	 * The UncalibratedSynapse features one output port of synaptic input from the synapses (port
	 * index 0) and one for the weight parameterization (port index 1).
	 */
