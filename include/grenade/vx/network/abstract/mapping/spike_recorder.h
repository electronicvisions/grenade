...
/**
 * Map from SpikeRecorder to CrossbarL2Output(s).
 */
struct SYMBOL_VISIBLE GENPYBIND(visible) SpikeRecorderMapping
    : public grenade::common::InterTopologyHyperEdge
{
	SpikeRecorderMapping(std::vector<signal_flow::SpikeFromChip> labels);

	std::vector<signal_flow::SpikeFromChip> labels;
	...
	virtual std::vector<std::vector<std::unique_ptr<grenade::common::PortData>>> map_output_data(
