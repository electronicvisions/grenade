...
/**
 * Map from ExternalSourceNeuron population to CrossbarL2Input(s).
 */
struct SYMBOL_VISIBLE GENPYBIND(visible) ExternalSourceNeuronMapping
    : public grenade::common::InterTopologyHyperEdge
{
	typedef std::vector<std::map<size_t, std::vector<halco::hicann_dls::vx::v3::SpikeLabel>>>
	    Labels;

	ExternalSourceNeuronMapping(Labels labels);
