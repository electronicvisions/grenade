...
GENPYBIND_MANUAL({
	[[maybe_unused]] ::grenade::vx::network::placed_logical::detail::RunUnrollPyBind11Helper<
	...
	parent.def(
	    ...
	    [](::pyhxcomm::Handle<execution::backend::Connection>& conn,
	       lola::vx::v3::Chip const& config,
	       network::placed_logical::NetworkGraph const& network_graph,
	       signal_flow::IODataMap const& inputs,
	       signal_flow::ExecutionInstancePlaybackHooks& playback_hooks) -> signal_flow::IODataMap {
		    return network::placed_logical::run(
	...
	parent.def(
	    ...
	    [](::pyhxcomm::Handle<execution::backend::Connection>& conn,
	       ...
	       network::placed_logical::NetworkGraph const& network_graph,
	       signal_flow::IODataMap const& inputs) -> signal_flow::IODataMap {
		    return network::placed_logical::run(conn.get(), config, network_graph, inputs);
	    },
	    pybind11::arg("connection"), pybind11::arg("config"), pybind11::arg("network_graph"),
	    pybind11::arg("inputs"));
