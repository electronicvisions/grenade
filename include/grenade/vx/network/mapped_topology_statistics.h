...
struct MappedTopologyStatistics;
struct NetworkGraph;

/**
 * Extract statistics from network graph.
 */
MappedTopologyStatistics GENPYBIND(visible)
    extract_statistics(grenade::common::LinkedTopology const& topology) SYMBOL_VISIBLE;

/**
 * Statistics of network graph.
 */
struct GENPYBIND(visible) MappedTopologyStatistics
{
	MappedTopologyStatistics() = default;
	...
	private:
		...
		friend MappedTopologyStatistics extract_statistics(
		    grenade::common::LinkedTopology const& topology);
	};

	std::map<grenade::common::ExecutionInstanceOnExecutor, ExecutionInstance> const&
	...
	GENPYBIND(stringstream)
	friend std::ostream& operator<<(std::ostream& os, MappedTopologyStatistics const& value)
	    SYMBOL_VISIBLE;

private:
	std::map<grenade::common::ExecutionInstanceOnExecutor, ExecutionInstance> m_execution_instances;
