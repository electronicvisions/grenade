...
struct GENPYBIND(visible) Population
{
	...
	bool enable_record_v{false};

	Population() = default;
	Population(Neurons const& neurons, bool enable_record_spikes, bool enable_record_v)
	    SYMBOL_VISIBLE;
	Population(Neurons&& neurons, bool enable_record_spikes, bool enable_record_v) SYMBOL_VISIBLE;
};
