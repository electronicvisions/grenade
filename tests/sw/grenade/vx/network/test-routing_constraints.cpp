...
TEST(RoutingConstraints, check)
{
	...
	// too many synapse rows on PADI-bus
	{
		...
		auto const synapse_rows = constraints.get_num_synapse_rows_per_padi_bus();
		EXPECT_THROW(constraints.check(), std::runtime_error);
	}
}
...
{
	RoutingConstraints::ExternalConnection connection{AtomicNeuronOnDLS(Enum(270)),
	                                                  std::pair{ProjectionDescriptor(), 0},
