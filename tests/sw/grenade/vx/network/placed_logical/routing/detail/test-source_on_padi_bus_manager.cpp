...
using namespace grenade::vx::network::placed_logical::routing::detail;
...
TEST(detail_SourceOnPADIBusManager, get_num_synapse_drivers)
{
	...
	for (auto& source : sources) {
		source.out_degree[grenade::vx::network::placed_logical::Receptor::Type::excitatory].fill(0);
	}
	sources.at(0).out_degree[grenade::vx::network::placed_logical::Receptor::Type::inhibitory].fill(
	    0);
	sources.at(0).out_degree[grenade::vx::network::placed_logical::Receptor::Type::inhibitory].at(
	    AtomicNeuronOnDLS(NeuronColumnOnDLS(), NeuronRowOnDLS(1))) = 17;
	sources.at(0).out_degree[grenade::vx::network::placed_logical::Receptor::Type::excitatory].at(
	    AtomicNeuronOnDLS(Enum(3))) = 12;
	sources.at(1).out_degree[grenade::vx::network::placed_logical::Receptor::Type::excitatory].at(
	    AtomicNeuronOnDLS(Enum(3))) = 10;
	sources.at(2).out_degree[grenade::vx::network::placed_logical::Receptor::Type::excitatory].at(
...
TEST(detail_SourceOnPADIBusManager, distribute_external_sources_linear)
{
	...
	for (size_t i = 0; i < 4; ++i) {
		sources.at(i)
		    .out_degree[grenade::vx::network::placed_logical::Receptor::Type::excitatory]
		    .fill(32);
	}
...
TEST(detail_SourceOnPADIBusManager, get_allocation_requests_internal)
{
	...
	std::vector<
	    grenade::vx::network::placed_logical::routing::SynapseDriverOnDLSManager::AllocationRequest>
	    expectation{
