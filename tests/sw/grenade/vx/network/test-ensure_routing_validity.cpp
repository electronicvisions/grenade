...
struct RandomNetworkGenerator
{
	    ...
	    std::vector<BackgroundSpikeSourcePopulation> const& background_populations)
	{
		...
		for (auto& projection : projections) {
			...
			//			std::bernoulli_distribution d(p_connection);
			//			for (size_t index_post = 0;
			//			     index_post < population_sizes.at(projection.population_post); ++index_post)
			//{ 				if (!support.at(projection.population_pre)[internal_populations.at(index)
			//				                                               .neurons.at(index_post)
			//				                                               .toNeuronRowOnDLS()
			//				                                               .toHemisphereOnDLS()]) {
			//					continue;
			//				}
			//				for (size_t index_pre = 0;
			//				     index_pre < population_sizes.at(projection.population_pre); ++index_pre)
			//{ 					if (d(m_rng)) { 						projection.connections.push_back(Projection::Connection( 						    index_pre,
			//index_post, Projection::Connection::Weight()));
			//					}
			//				}
			//			}
		}
