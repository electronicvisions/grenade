...
TEST(update_network_graph, ProjectionOnBothHemispheres)
{
	...
	Projection projection(
	    ...
	    {{0, 0, grenade::vx::network::placed_atomic::Projection::Connection::Weight(63)},
	     {0, 1, grenade::vx::network::placed_atomic::Projection::Connection::Weight(63)},
	     {1, 0, grenade::vx::network::placed_atomic::Projection::Connection::Weight(63)},
	     {1, 1, grenade::vx::network::placed_atomic::Projection::Connection::Weight(63)}},
	...
	for (auto& connection : projection.connections) {
		connection.weight = grenade::vx::network::placed_atomic::Projection::Connection::Weight(0);
	}
	...
	EXPECT_NO_THROW(
	    grenade::vx::network::placed_atomic::update_network_graph(network_graph, new_network));
}
...
TEST(build_network_graph, GranularitySweep)
{
	...
	for (size_t num_pops = 1; num_pops <= NeuronColumnOnDLS::size; num_pops *= 2) {
		...
		for (size_t i = 0; i < num_pops; ++i) {
			grenade::vx::network::placed_atomic::ExternalPopulation population_external{
			    neuron_per_pop};
