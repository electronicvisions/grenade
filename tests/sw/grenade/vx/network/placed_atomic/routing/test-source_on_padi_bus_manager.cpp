...
using namespace grenade::vx::network::placed_atomic::routing;
...
TEST(SourceOnPADIBusManager, AllToAllInternalTop)
{
	...
	for (auto const column : iter_all<NeuronColumnOnDLS>()) {
		...
		source.out_degree[Projection::ReceptorType::excitatory].fill(1);
...
TEST(SourceOnPADIBusManager, Background128)
{
	...
	for (size_t i = 0; i < 128; ++i) {
		SourceOnPADIBusManager::BackgroundSource source;
		source.out_degree[Projection::ReceptorType::excitatory].fill(0);
		source.out_degree[Projection::ReceptorType::excitatory][AtomicNeuronOnDLS(Enum(i))] = 1;
		source.padi_bus = PADIBusOnDLS();
		background_sources.push_back(source);
	}
