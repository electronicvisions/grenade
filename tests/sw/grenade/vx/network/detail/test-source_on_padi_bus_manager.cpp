...
TEST(detail_SourceOnPADIBusManager, get_num_synapse_drivers)
{
	...
	for (auto& source : sources) {
		source.out_degree[grenade::vx::network::Projection::ReceptorType::excitatory].fill(0);
	}
	sources.at(0).out_degree[grenade::vx::network::Projection::ReceptorType::inhibitory].fill(0);
	sources.at(0).out_degree[grenade::vx::network::Projection::ReceptorType::inhibitory].at(
	    AtomicNeuronOnDLS(NeuronColumnOnDLS(), NeuronRowOnDLS(1))) = 17;
	sources.at(0).out_degree[grenade::vx::network::Projection::ReceptorType::excitatory].at(
	    AtomicNeuronOnDLS(Enum(3))) = 12;
	sources.at(1).out_degree[grenade::vx::network::Projection::ReceptorType::excitatory].at(
	    AtomicNeuronOnDLS(Enum(3))) = 10;
	sources.at(2).out_degree[grenade::vx::network::Projection::ReceptorType::excitatory].at(
	    AtomicNeuronOnDLS(Enum(5))) = 5;
...
TEST(detail_SourceOnPADIBusManager, distribute_external_sources_linear)
{
	...
	for (size_t i = 0; i < 4; ++i) {
		sources.at(i).out_degree[grenade::vx::network::Projection::ReceptorType::excitatory].fill(
		    32);
	}
	for (size_t i = 4; i < 68; ++i) {
		sources.at(i).out_degree[grenade::vx::network::Projection::ReceptorType::excitatory].fill(
		    0);
		sources.at(i).out_degree[grenade::vx::network::Projection::ReceptorType::excitatory].at(
		    AtomicNeuronOnDLS(Enum(i))) = 1;
	}
...
TEST(detail_SourceOnPADIBusManager, get_allocation_requests_internal)
{
	...
	std::vector<grenade::vx::network::SynapseDriverOnDLSManager::AllocationRequest> expectation{
...
TEST(detail_SourceOnPADIBusManager, get_allocation_requests_external)
{
	...
	grenade::vx::network::SynapseDriverOnDLSManager::AllocationRequest expectation{
	    {{PADIBusOnDLS(Enum(2)), {{12, false}}}}, {}, std::nullopt};
