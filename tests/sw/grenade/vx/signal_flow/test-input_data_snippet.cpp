...
TEST(InputDataSnippet, General)
{
	InputDataSnippet map;
	...
	InputDataSnippet map_move(std::move(map));
	EXPECT_EQ(data, map_move.data.at(0));
	EXPECT_EQ(runtime, map_move.runtime);

	InputDataSnippet map_move_2 = std::move(map_move);
...
TEST(InputDataSnippet, is_match)
{
	auto const data = std::vector<TimedDataSequence<std::vector<Int8>>>{
	    {{common::Time(), {Int8(5), Int8(6)}}}, {{common::Time(), {Int8(7), Int8(8)}}}};
	{
		Port port(2, ConnectionType::Int8);
		EXPECT_TRUE(InputDataSnippet::is_match(data, port));
	}
	{
		Port port(3, ConnectionType::Int8);
		EXPECT_FALSE(InputDataSnippet::is_match(data, port));
	}
	{
